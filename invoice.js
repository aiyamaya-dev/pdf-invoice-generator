...
const STATUS_COLORS = { draft: '#94a3b8', sent: '#3b82f6', paid: '#22c55e', overdue: '#ef4444' };
...
function generateInvoicePDF(data) {
  ...
  if (data.status) {
    ...
    doc.fontSize(11).fillColor(color).text(data.status.toUpperCase(), 450, 55, { align: 'right' });
  }
  ...
  if (discount) {
    y += 20;
    doc.fillColor('#64748b').text('Discount:', 380, y);
    doc.fillColor('#ef4444').text(`-${currency}${discount.toFixed(2)}`, 470, y);
  }
  ...
  // Footer
  doc.fontSize(8).fillColor('#94a3b8').text('Generated by NovaTech Solutions — Invoice Management System', 50, 750, { align: 'center' });
